...
```
Again using lm() to fit a multiple regression model var2 with 2 new predictors like bedrooms and bath_full_count. This is the second variable var2. We are trying to predict sale price as a function of both bath_full_count and Bedrooms.
...
tolerance
```
The VIF and tolerance statistics are useful statistics to assess collinarity. For our current model the VIF values for both the variables are all well below 10 and the tolerance statistics all well above 0.2. Based on these measures we can safely conclude that there is no collinearity within our data.
...
```
From the above Residuals vs Fitted plot it shows that the scatter points are not random and rather concentrated towards the positive values for the residuals. Hence the data shows a non linear relationship.
The QQ plot shows deviation from linearity. The normal distribution shown by the stariaght line do not have all the points over it so it shows skewness of the data towards the higher end of the model.
...
```
The histogram for the data is roughly skewed to the right as it does not have a bell-curve shape.
...
So overall this regression model is not unbiased. It is not a perfectly good model for the data as it has outliers,influential cases, etc.and so we cannot use that model to draw conclusions about our sample. We cannot generalize your findings beyond your sample to the entire population.
